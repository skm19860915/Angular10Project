  ...
  loadInfluencerListForBrand() {
    this.route.queryParams.subscribe(query => {
          ...
          if (!this.cselection.selected.length){
              ...
              this.mselection.selected.forEach(m => {
                  this.cselection.selected.forEach(c => {
                      ...
                      .pipe()
                      .subscribe((cdata : any) => {
                        console.log('send offer result', cdata);
                        var chat: Chat = {
                          id: 3,
              ...
              console.log('send offer', this.mselection, this.campService.offers);
      ...
      saveFavorite(id?: Profile['id']){
        ...
        if (this.favorites.length > 0){
          ...
          if (index !== -1) {
            ...
            let profile = this.favorites.find(c => c.id === id);  
            if(profile){
          ...
          this.campService.saveInfluencerToFavoriteList(favorite_id, id );
          let profile = this.profiles.find(c => c.id === id);
          profile.favorited = true;
          this.favorites.push(profile);
        }
      }
      ...
      saveMylist(id?: Profile['id']){
        let my_list_id = this.campService.influencerMyListId;
