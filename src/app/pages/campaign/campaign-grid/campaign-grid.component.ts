...
export class CampaignGridComponent implements OnInit, AfterViewChecked {
  ...
  filteredCampaigns$ = this.route.queryParamMap.pipe(
      map(paramMap => {
        console.log('filter', this.campService.campList);
        ...
        console.log('filter2', this.campaigns);

        switch (this.activeTab) {
          case 'storage': {
            this.campaigns = show === 'all' ? this.campaigns.filter(c => !c.favorite) :
                this.campService.filter(show);
            console.log('filter1', this.campaigns, this.campaigns.filter(c => !c.favorite), this.campService.filter(show));
  ...
  campListInit() {
    ...
    .subscribe((cdata : any) => {
      for(let i = 0; i < cdata.results.length; i ++) {
        ...
        console.log('campaign item ', i, cdata.results[i], cdata.results[i].campaign_uuid);
      ...
      console.log(this.campService.campList);
    })
  }
  ngOnInit(): void {
